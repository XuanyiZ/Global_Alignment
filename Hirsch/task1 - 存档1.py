...
def calculate_currow(prev_row, n, seq1, seq2, prev_row_idx):
	...
	for k in range(1, n+1):
		...
		match_diagonal = prev_row[0][k-1] + int(find_score(seq1[prev_row_idx +1 -1], seq2[k-1]))
		curr_row[0][k] = max(insert_horizontal, delete_vertical, match_diagonal)
	return curr_row


def godown_nextrow(prev_row, curr_row, n, seq1, seq2):
...
def align(seq1, seq2):
	...
	#Induction Rule
	align1 = ''  # alignment for seq1
	align2 = ''
	i = 1  # current row idx position
	j = 1  # current col idx position
	while (i <= m and j <= n):
		#insert_horizontal
		if (curr_row[0][j] == curr_row[0][j-1] + gap_penalty):
			align1 = align1 + '-'
			align2 = align2 + seq2[j-1]
			i = i + 1
			continue;
		#delete_vertical
		elif (curr_row[0][j] == prev_row[0][j] + gap_penalty):
			align1 = align1 + '-'
			align2 = align2 + seq2[j-1]
			j = j + 1
			prev_row, curr_row = godown_nextrow(prev_row, curr_row, n, seq1, seq2)
			prev_row_idx = prev_row_idx + 1	
			continue;
		#match_diagonal
		elif (curr_row[0][j] == prev_row[0][j-1] + int(find_score(seq1[i-1], seq2[j-1]))):
